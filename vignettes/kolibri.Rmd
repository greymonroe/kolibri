...
vignette: >
  %\VignetteIndexEntry{Introduction to kolibri}
...
```{r read-bed}
bed12_path <- system.file("extdata", "alignVA2.bed12", package = "kolibri")
bed12 <- read_bed12(bed12_path)
head(bed12)
```
...
If you also have the genome loaded (e.g. with `seqinr::read.fasta()`), you can pull out the actual sequence span for the cluster(s).

```{r nodes-to-fasta, eval=FALSE}
genome_path <- system.file("extdata", "Col-CEN_v1.2.fasta.gz",
                           package = "kolibri")
genome <- seqinr::read.fasta(genome_path)
